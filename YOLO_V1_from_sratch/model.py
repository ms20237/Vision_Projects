import torch
import torch.nn as nn


architecture_config = [
    (7, 64, 2, 3),
    "M",
    (3, 192, 1, 1),
    "M",
    (1, 128, 1, 0),
    (3, 256, 1, 1),
    (1, 256, 1, 0),
    (3, 512, 1, 1),
    "M",
  # ()...., num_repeats)
    [(1, 256, 1, 0), (3, 512, 1, 1), 4],
    (1, 512, 1, 0),
    (3, 1024, 1, 1),
    "M",
    [(1, 512, 1, 0), (3, 1024, 1, 1), 2],
    (3, 1024, 1, 1),
    (3, 1024, 2, 1),
    (3, 1024, 1, 1),
    (3, 1024, 1, 1),
    # (kernel_size, num_filter, stride, padding)
]

# creat block for model
class CNN_Block(nn.Module):
    def __init__(self, in_channels, out_channels, **kwargs):
        super(CNN_Block, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, bias=False, **kwargs)
        self.batch_nom = nn.BatchNorm2d(out_channels)
        self.leaky_relu = nn.LeakyReLU(0.1)

 
    def forward(self, x):
        return self.leaky_relu(self.batch_nom(self.conv1(x)))


# implement model
class YOLO_V1(nn.Module):
    def __init__(self, in_channels=3, Split_size=7, num_boxes=2, num_classes=20, **kwargs):
        super(YOLO_V1, self).__init__()
        self.architecture = architecture_config
        self.in_channels = in_channels
        self.darknet = self._create_conv_layers(self.architecture)
        self.fcs = self._create_fcs(**kwargs)

    def forward(self, x):
        x = self.darknet(x)
        return self.fcs(torch.flatten(x, start_dim=1))

    def _create_conv_layers(self, architecture):
        layers = []
        in_channels = self.in_channels

        for x in architecture:
            if type(x) == tuple:
               layers += [CNN_Block(
                    in_channels, x[1], kernel_size=x[0], stride=x[2], padding=x[3],
                )
           ]
               in_channels = x[1] # Update in_channels after each convolutional block

            elif type(x) == str:
                layers += [nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))]

            elif type(x) == list:
                conv1 = x[0] # tuple
                conv2 = x[1] # tuple
                num_repeats = x[2] # Integer

                for _ in range(num_repeats):
                    layers += [
                        CNN_Block(
                            in_channels=in_channels,
                            out_channels=conv1[1],
                            kernel_size=conv1[0],
                            stride=conv1[2],
                            padding=conv1[3],
                        )
                    ]
                    layers += [
                        CNN_Block(
                            in_channels=conv1[1],
                            out_channels=conv2[1],
                            kernel_size=conv2[0],
                            stride=conv2[2],
                            padding=conv2[3]
                        )
                    ]
                    in_channels = conv2[1] # Update in_channels after each convolutional block
        return nn.Sequential(*layers)

    def _create_fcs(self, Split_size=7, num_boxes=2, num_classes=20):
        S, B, C = Split_size, num_boxes, num_classes
        return nn.Sequential(
            nn.Flatten(),
            nn.Linear(in_features=1024 * S * S, out_features=496),
            nn.Dropout(0.0),
            nn.LeakyReLU(0.1),
            nn.Linear(in_features=496, out_features=S * S * (C + B * 5)), # (S,S,30) where C+B*5=30
        )

# # test example
# def test(S=7, B=2, C=20):
#     model = YOLO_V1(Split_size=S, num_boxes=B, num_classes=C)
#     x = torch.randn(2, 3, 448, 448) # Channels should be in the second dimension
#     print(model(x).shape)

# test()
# # output = [2, 7*7*30]
    
    
    
    