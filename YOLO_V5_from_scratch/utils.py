...
def non_max_suppression(bboxes, iou_threshold, threshold, box_format="corners"):
    ...
    exactly what cellboxes_to_boxes produces once it's pooled boxes
...
# --------------------------------------------------------------------- #
#  Multi-scale box decoding & target building                          #
# --------------------------------------------------------------------- #
#
# YOLOv2's decoders (convert_cellboxes_v2 / cellboxes_to_boxes_v2) assumed
# a single S x S grid with a single anchor set. Both YOLOv3 and YOLOv4
# predict at 3 grids simultaneously with different anchors per grid (v4
# only changes *how* those 3 grids are produced -- CSPDarknet53 + SPP +
# PANet instead of Darknet53 + a plain FPN -- not the grid/anchor
# convention itself), so decoding needs to happen per-scale and then be
# pooled together before NMS -- a detection from the coarse 13x13 grid
# competes on equal footing with one from the fine 52x52 grid once both
# are expressed in whole-image-relative units. Everything below this
# point is unchanged between the YOLOv3 and YOLOv4 versions of this file.

def decode_scale_predictions(pred, anchors_px, base_size, C=20):
    """
...
def cellboxes_to_boxes(predictions, anchors, base_size=416, C=20):
    """
    List-of-lists version of decode_scale_predictions, pooling boxes
    ...
    """
    all_scale_boxes = [
        decode_scale_predictions(pred, anch, base_size, C=C)
...
def build_targets(boxes_per_image, anchors, grid_sizes, base_size=416, C=20, device="cpu"):
    """
...
def yolo_multiscale_collate_fn(batch):
    """
    Custom collate_fn for a YOLOv3/v4-style multi-scale dataset. Unlike YOLOv1/v2, targets are
    ...
    that batch). Binning happens in build_targets at train/eval time.
    """
...
def get_bboxes(
    ...
    YOLOv3/v4's list-of-3-scales and raw-box-list targets (see
    yolo_multiscale_collate_fn) instead of a single pre-gridded target tensor.
    """
    all_pred_boxes = []
    all_true_boxes = []
    ...
    for batch_idx, (x, boxes) in enumerate(loader):
        ...
        pred_boxes = cellboxes_to_boxes(predictions, anchors, base_size=base_size, C=C)
...
def train_fn(
    ...
    lists (build_targets) rather than pre-gridded by the Dataset,
    ...
    for batch_idx, (x, boxes) in enumerate(loop):
        ...
        targets = build_targets(boxes, anchors, grid_sizes, base_size=base_size, C=C, device=device)
