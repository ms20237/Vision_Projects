...
from loss_f import YOLO_V5_LOSS
from model import YOLO_V5, COCO_ANCHORS
...
DEFAULT_CONFIG_PATH = "./YOLO_V5_from_sratch/configs/train_config.yaml"

# 416 is the standard YOLOv5 base resolution; the 3 output grids
...
def init():
    ...
    """
    parser = argparse.ArgumentParser(description="YOLO V5 Training Script")
...
def main(img_dir: str,
    ...
    model = YOLO_V5(num_classes=C).to(device)

    optimizer = optim.Adam(
        model.parameters(), lr=lr, weight_decay=weight_decay)

    loss_fn = YOLO_V5_LOSS(C=C, anchors=anchors, base_size=BASE_INPUT_SIZE)
